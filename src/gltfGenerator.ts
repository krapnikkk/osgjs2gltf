...
import { Accessor, glTF, Material } from "../@types/gltf";
 ...
 * @private
 */
function gltfGenerator(objData, options) {
    ...
    for (let i = 0; i < nodesLength; ++i) {
        ...
        if (meshesLength === 1) {
            ...
            const parentIndex = addNode(gltf, node.name, 0);
...
function addMesh(gltf:glTF, materials:Material, bufferState, uint32Indices, mesh, options) {
...
function addNode(gltf: glTF, name: string, meshIndex: number, parentIndex?: number) {
    const node = {
        name: name,
        mesh: meshIndex,
    };
